...
CREATE INDEX `fk_Codigo_Curso1_idx` ON `WebPlants`.`Codigo` (`Curso_idCurso` ASC) VISIBLE;
...
USE `WebPlants`$$
DROP TRIGGER IF EXISTS `WebPlants`.`Curso_AFTER_INSERT` $$
USE `WebPlants`$$
CREATE DEFINER = CURRENT_USER TRIGGER `WebPlants`.`Curso_AFTER_INSERT` AFTER INSERT ON `Curso` FOR EACH ROW
BEGIN
INSERT INTO `Codigo` (`Codigo`.`codigo`, `Codigo`.`estado`, `Codigo`.`Curso_idCurso`)
VALUES  
((select lpad(conv(floor(Rand()*pow(36,8)), 10, 36), 8, 0) as rnd_str_4z),'Activo',NEW.idCurso);
END$$
